...
type Row = { runs: Run[]; size: number; height: number };
type Block = { rows: Row[]; height: number; sing: boolean; number: number };

function font(size: number, bold = false) { return `${bold ? 800 : 400} ${size}px ${FONT}`; }

function wrap(ctx: CanvasRenderingContext2D, tokens: LyricToken[], width: number, size: number, color: string): Row[] {
  const rows: Row[] = [];
  let runs: Run[] = [], used = 0;
  const flush = () => { rows.push({ runs, size, height: size * 1.5 }); runs = []; used = 0; };
  for (const token of tokens) {
    const bold = Boolean(token.call), ink = bold ? CALL : color;
    ctx.font = font(size, bold);
    for (const char of Array.from(token.text)) {
      if (char === "\n") { flush(); continue; }
      const length = ctx.measureText(char).width;
      if (used + length > width && runs.length) flush();
      const last = runs.at(-1);
      if (last && last.bold === bold && last.color === ink) last.text += char;
      else runs.push({ text: char, bold, color: ink });
      used += length;
    }
  }
...
export async function renderGuideImage(song: SongGuideData, sync: PublishedSongSync | undefined, translation: boolean): Promise<Blob> {
  ...
  const blocks: Block[] = portableLines(song, sync).map((line, index) => {
    ...
    for (const cue of line.cues) {
      ...
      rows.push(...wrap(ctx, [{ text: annotation, call: true }], contentWidth, 21, CALL));
    }
    rows.push(...wrap(ctx, line.original, contentWidth, 24, MUTED));
    rows.push(...wrap(ctx, line.pronunciation, contentWidth, 29, INK));
    if (translation) rows.push(...wrap(ctx, line.translation, contentWidth, 22, MUTED));
  ...
  const titleRows = wrap(ctx, [{text: song.title}], width - margin * 2, 76, INK);
  ...
  for (const row of titleRows) {
    ctx.font = font(row.size, true); ctx.fillStyle = INK;
    ...
    for (const block of group) {
      ...
      for (const row of block.rows) {
        ...
        for (const run of row.runs) {
          ctx.font = font(row.size, run.bold); ctx.fillStyle = run.color;
          // Per-character drawing matches the wrapping measurements exactly.
          for (const char of Array.from(run.text)) { ctx.fillText(char, runX, rowY); runX += ctx.measureText(char).width; }
        }
        rowY += row.height;
      }
      y += block.height;
